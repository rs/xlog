...
func TestLevelMarshalerText(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, levelBytesDebug, string(b))
	b, err = LevelInfo.MarshalText()
	assert.NoError(t, err)
	assert.Equal(t, levelBytesInfo, string(b))
	b, err = LevelWarn.MarshalText()
	assert.NoError(t, err)
	assert.Equal(t, levelBytesWarn, string(b))
	b, err = LevelError.MarshalText()
	assert.NoError(t, err)
	assert.Equal(t, levelBytesError, string(b))
	b, err = LevelFatal.MarshalText()
	assert.NoError(t, err)
	assert.Equal(t, levelBytesFatal, string(b))
